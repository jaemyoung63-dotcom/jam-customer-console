...
export async function onRequestPost(context) {
  ...
  if (payload.mode === 'tidy') {
    ...
    const tidySystem = [
      ...
      "- 텍스트에 [보장급부], [내보장자산] 구분 표시가 있으면 유지하고, 각 항목을 '항목명 금액' 형태의 짧은 줄로 정리하세요.",
      '- 구분 표시가 없으면 내용으로 판단해 [보장급부]와 [내보장자산]으로 나누되, 애매하면 [기타]로 두세요.',
      '- OCR 오류로 보이는 잡음은 제거하되, 숫자와 금액은 원문 그대로 두고 바꾸지 마세요.',
      '- 원문에 없는 항목이나 금액을 새로 지어내지 마세요.',
      '- 출력은 정리된 텍스트만. 설명 없이 결과만 출력하세요.'
    ].join('\n');
    try {
      ...
      if (!rr.ok) return json({ error: (dd && dd.error && dd.error.message) ? dd.error.message : ('API 오류 (' + rr.status + ')') }, rr.status);
      const tidied = (dd.content || []).filter(b => b.type === 'text').map(b => b.text).join('\n').trim();
      return json({ text: tidied, _usage: { input_tokens: (dd.usage && dd.usage.input_tokens) || 0, output_tokens: (dd.usage && dd.usage.output_tokens) || 0, model: dd.model || model } });
    } catch (err) {
  ...
  const system = [
    ...
    '반드시 아래 JSON 형식 하나만 출력하세요. 마크다운, 코드블록, 설명 없이 순수 JSON만.',
    '{ "summary": "종합 요약 2~3문장", "areas": [{"name":"영역명","level":"충분|보통|취약","reason":"한 줄 근거"}], "priorities": ["보강 제안 1","보강 제안 2","보강 제안 3"], "detail": "상세 분석. 문단으로 구분하고 구체적으로." }',
    '- 영역은 사망보장, 암·3대진단, 입원·수술, 실손의료, 간병·치매, 상해·재해 중 텍스트에서 확인되는 것을 다룹니다.',
    "- 관련 담보가 전혀 없으면 그 영역을 '취약'으로 판정하고 reason에 '해당 담보 없음'을 명시합니다.",
    '- 텍스트에 없는 보험료·환급금 숫자는 지어내지 마세요.'
  ].join('\n');
  const user = [
    '# 고객 정보',
    ...
    '', '# 보장 텍스트', coverageText, '', '# 참고 상담사례', caseText
  ].join('\n');
  try {
    ...
    if (!r.ok) return json({ error: (data && data.error && data.error.message) ? data.error.message : ('API 오류 (' + r.status + ')') }, r.status);
    const text = (data.content || []).filter(b => b.type === 'text').map(b => b.text).join('\n').trim();
    let parsed;
    try { parsed = JSON.parse(text); }
    catch (e) { const m = text.match(/\{[\s\S]*\}/); parsed = m ? JSON.parse(m[0]) : { summary: text, areas: [], priorities: [], detail: text }; }
    parsed._usage = { input_tokens: (data.usage && data.usage.input_tokens) || 0, output_tokens: (data.usage && data.usage.output_tokens) || 0, model: data.model || model };
    return json(parsed);
  } catch (err) {
