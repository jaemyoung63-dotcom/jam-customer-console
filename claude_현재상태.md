...
4. **홈화면 6탭 구조 설계 논의 + 1단계 구현** — TA-고객정보-참조풀-분석-상담-고객관리 6탭으로 재배치, 홈은 하단 탭에서 빼서 상단 우측 아이콘으로 이동. 자세한 설계·UX의견·남은 결정사항은 `docs/6탭구조_설계메모.md` 참고.
   - 실제 코드 변경: `index.html`(탭바 6개로 교체, `s-ta`·`s-customermgmt` 화면 자리 추가), `core.js`(`header()` 홈버튼 우측 이동, `go()`에 ta/customermgmt 라우팅 + `renderTA`/`renderCustomerMgmt` 준비중 placeholder 추가), `style.css`(탭 라벨 폭 조정).
   - TA·고객관리는 아직 "준비중" 빈 화면만 있음 — 실제 기능(TA 자료 관리, 히스토리 타임라인, Q&A)은 다음 단계.

## 아직 남은 것 / 확인 필요
- **6탭 구조 2·3·4단계** — `docs/6탭구조_설계메모.md`의 진행 순서 참고 (TA 관리자화면 → 고객관리 히스토리 → Q&A). 구글드라이브 연동, TA 자료 항목 구성 등 미정 사항도 그 문서에 정리됨.
