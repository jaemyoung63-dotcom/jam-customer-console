...
## 아직 남은 것 / 확인 필요
- **6탭 구조 4단계(Q&A 코너)** — `docs/6탭구조_설계메모.md` 참고. 히스토리가 어느 정도 쌓인 뒤 진행 추천.
- **TA·고객관리·구글드라이브 실사용 테스트 필요** — Push 후 ①⚙ 관리자 화면 → TA 관리에서 자료 등록·공개 체크 → 담당자 화면 TA 탭 확인, ②고객관리 탭에서 메모/음원 넣고 AI 정리 → 히스토리에 잘 쌓이는지, ③AP 상담결과 저장 후 고객관리에 자동으로 기록이 남는지, ④"☁️ 구글드라이브에서 가져오기"로 실제 음원 파일 하나 가져와보기(처음 누르면 구글 로그인·권한 동의 화면이 뜨는 게 정상).
