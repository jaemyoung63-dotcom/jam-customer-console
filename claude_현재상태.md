...
- **유료 실측 2건 (앱11부터 이월)**: ①음원 받아쓰기(짧은 실음원) ②성별 여성 보장분석(실보장 이미지로 여성 기준 서술 확인). 아직 실측 기록 없음.
- **"참조풀 매칭"/"참조 자료" 중복 섹션 정리 여부** — 위 앱13 메모 참고, jam님께 여쭤보고 필요하면 하나로 합치기.
- 테스트로 만든 고객 **"테스트고객"**(id `mtbnuaiasw995`) — 앱11 세션에서 생성, 아직 남아있으면 정리 시 삭제 가능.
