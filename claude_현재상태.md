...
- **구글드라이브 연동** — 실제로 붙이려면 Google Cloud Console에서 OAuth 클라이언트 설정이 필요(jam님이 해야 하는 부분). 원하시면 설정 방법부터 안내 가능.
- **TA·고객관리 실사용 테스트 필요** — Push 후 ①⚙ 관리자 화면 → TA 관리에서 자료 등록·공개 체크 → 담당자 화면 TA 탭 확인, ②고객관리 탭에서 메모/음원 넣고 AI 정리 → 히스토리에 잘 쌓이는지, ③AP 상담결과 저장 후 고객관리에 자동으로 기록이 남는지 확인.
